 ...
 */
(function () {
  // Ported into Byggeapp: the reference loaded three.js from unpkg, the app
  // bundles it from node_modules (see _boot). Keep every other line in sync
  // with docs/HOUSE_MODEL_HANDOFF.md.
  ...
  /* Ported: the reference generated every surface on canvas so it could run
     offline. In the app the same families are backed by the Poly Haven maps in
     public/textures/house — the procedural set stays as the first paint and as
     the fallback if a map 404s. Densities live on the family spec and are
     applied per geometry in _boxGeo, so the swap needs no UV changes.
     `tiles` (paving) has no licensed pair yet and stays procedural. */
  const SURFACE_MAPS = {
    roof: 'grey_roof_tiles',
    brick: 'white_bricks',
    wood: 'wooden_planks',
    deck: 'wooden_planks',
    floor: 'wooden_planks',
  ...
  /* Two of the shipped maps fight the model's design language: `white_bricks`
     is a weathered grey-green wall rather than clean Nordic brick, and
     `leafy_grass` is autumn leaf litter rather than lawn. Both are graded on a
     canvas before upload — the photographic detail survives, the colour comes
     back to the reference. Delete an entry to use a map exactly as shipped. */
  const SURFACE_GRADE = {
    brick: { brightness: 1.3, contrast: 0.72, saturate: 0.26 },
    grass: { brightness: 1.04, contrast: 1.05, saturate: 1.5, hue: -16 }
  };

  /* The grade is done with pixel maths rather than ctx.filter: WebKit — i.e.
     every browser on iOS — has no canvas filter before Safari 18, and a silently
     ungraded map there means the facade renders as weathered grey on iPhone but
     clean brick everywhere else. Matrices follow the Filter Effects spec so the
     result matches the equivalent CSS filter chain. */
  const mul3 = (A, B) => {
  ...
  const gradeMatrix = (g) => {
    const s = g.saturate == null ? 1 : g.saturate;
    const sat = [
    ...
    if (!g.hue) return sat;
    const a = g.hue * Math.PI / 180, c = Math.cos(a), n = Math.sin(a);
    const hue = [
  ...
  class HouseStage extends HTMLElement {
    ...
    async _boot() {
      // Bumped on every boot and teardown so async work (three, surface maps)
      // can tell whether it is still working for the current scene.
      this._bootToken = (this._bootToken || 0) + 1;
      ...
      this.invalidate(3);
      // Ported: the element can be removed while _boot is still awaiting three,
      // in which case there is nothing left to announce.
      if (!this.isConnected && !this._booted) { this._teardown(); return; }
      window.__houseStage = this;
      window.dispatchEvent(new CustomEvent('housestage:ready', { detail: { stage: this } }));
      // Progressive: the procedural set is already on screen, the photographic
      // maps swap in when they land.
      this._loadSurfaceMaps();
      // Ported: when the host creates the element in JS the first ResizeObserver
      // notification can land before layout, so _boot's 960x600 fallback would
      // stick. Re-measure once the frame after the element is laid out.
      requestAnimationFrame(() => requestAnimationFrame(() => this._resize()));
    }

    /* Ported: the app mounts and unmounts the stage with the wizard step, so the
       renderer, the animation loop and the resize observer have to go with it. */
    disconnectedCallback() {
      ...
      this._overlay = null;
      // _inputBound stays true on purpose: the pointer listeners live on the
      // element itself, so a re-connect must not bind a second set.
      if (window.__houseStage === this) window.__houseStage = null;
    }
    ...
    _makeMaterials() {
      ...
      this._surfaceTex = [];   // loaded Poly Haven maps, disposed on teardown
    ...
    /* Ported: swap the procedural family maps for the Poly Haven set once it
       loads. Høj takes the JPEG pair, Mobil the smaller WebP pair — the same
       split the app used before, and setQuality reboots so the profile is
       always in step. A family whose maps fail to load keeps its canvas
       textures, so a missing file degrades instead of breaking. */
    async _loadSurfaceMaps() {
      const T = this.T, token = this._bootToken;
      if (!T || !this.renderer) return;
      const lite = this.quality === 'mobil';
      const base = TEXTURE_BASE + 'textures/house' + (lite ? '/mobile' : '') + '/';
      const ext = lite ? 'webp' : 'jpg';
      const aniso = Math.min(this.P.aniso, this.renderer.capabilities.getMaxAnisotropy());
      const loader = new T.TextureLoader();
      const load = (url, srgb) => new Promise((resolve) => {
        loader.load(url, (t) => {
          if (srgb) t.colorSpace = T.SRGBColorSpace;   // normal/rough stay linear
          t.wrapS = t.wrapT = T.RepeatWrapping;
          t.anisotropy = aniso;
          resolve(t);
      ...
      const grade = (tex, g) => {
        ...
        x.drawImage(img, 0, 0);
        let data;
        try { data = x.getImageData(0, 0, c.width, c.height); } catch (e) { return tex; }
        const px = data.data;
        const b = g.brightness == null ? 1 : g.brightness;
        const ct = g.contrast == null ? 1 : g.contrast;
        const lut = new Uint8ClampedArray(256);
        for (let i = 0; i < 256; i++) lut[i] = ((i / 255 * b - 0.5) * ct + 0.5) * 255;
        const m = gradeMatrix(g);
        for (let i = 0; i < px.length; i += 4) {
          const r = lut[px[i]], gg = lut[px[i + 1]], bb = lut[px[i + 2]];
          px[i] = m[0] * r + m[1] * gg + m[2] * bb;
          px[i + 1] = m[3] * r + m[4] * gg + m[5] * bb;
          px[i + 2] = m[6] * r + m[7] * gg + m[8] * bb;
        }
        x.putImageData(data, 0, 0);
        const out = new T.CanvasTexture(c);
        out.colorSpace = tex.colorSpace;
        out.wrapS = out.wrapT = T.RepeatWrapping;
        out.anisotropy = tex.anisotropy;
        tex.dispose();
        return out;
      };

      const packs = await Promise.all(Object.keys(SURFACE_MAPS).map(async (famKey) => {
        const prefix = SURFACE_MAPS[famKey];
        const [map, normalMap, roughnessMap] = await Promise.all([
          load(base + prefix + '-diff.' + ext, true),
          load(base + prefix + '-normal.' + ext, false),
          load(base + prefix + '-rough.' + ext, false)
        ]);
        return { famKey, map: grade(map, SURFACE_GRADE[famKey]), normalMap, roughnessMap };
      }));

      const dispose = (list) => list.forEach((t) => { if (t && t.dispose) t.dispose(); });
      // torn down or rebooted (quality switch) while the maps were in flight
      if (token !== this._bootToken || !this.renderer || !this._matCache) {
        packs.forEach((p) => dispose([p.map, p.normalMap, p.roughnessMap]));
        return;
      }

      const retired = [];
      packs.forEach((pack) => {
        const fam = this.fam[pack.famKey];
        if (!fam || !pack.map) { dispose([pack.map, pack.normalMap, pack.roughnessMap]); return; }
        retired.push(fam.map);
        fam.map = pack.map;
        if (pack.normalMap) { retired.push(fam.normalMap); fam.normalMap = pack.normalMap; }
        fam.roughnessMap = pack.roughnessMap || null;
        this._surfaceTex.push(pack.map, pack.normalMap, pack.roughnessMap);
      });

      // re-point the materials that were already built from the procedural set
      this._matCache.forEach((m, key) => {
        const fam = this.fam[key.slice(0, key.indexOf('|'))];
        if (!fam || !fam.map) return;
        m.map = fam.map;
        m.normalMap = fam.normalMap;
        m.roughnessMap = fam.roughnessMap || null;
        m.needsUpdate = true;
      });
      dispose(retired);
      this.renderer.shadowMap.needsUpdate = true;
    ...
    _teardown() {
      this._bootToken = (this._bootToken || 0) + 1;   // orphan any in-flight loads
      if (this._surfaceTex) this._surfaceTex.forEach((t) => { if (t && t.dispose) t.dispose(); });
      this._surfaceTex = null;
